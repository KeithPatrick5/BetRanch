...
export const GAME_KEYS: GameKey[] = ["dice", "limbo", "mines", "plinko", "wheel", "keno", "hilo", "tower", "crash", "blackjack", "rps", "war"];
...
export function numberParam(params: Record<string, unknown> | undefined, key: string, fallback: number, min: number, max: number) {
  const raw = Number(params?.[key] ?? fallback);
  if (!Number.isFinite(raw)) return fallback;
  return Math.min(max, Math.max(min, raw));
}

export function stringParam<T extends string>(params: Record<string, unknown> | undefined, key: string, fallback: T, allowed: readonly T[]) {
...
export function runGame(game: GameKey, wager: number, fair: FairInput, params?: Record<string, unknown>): GameOutcome {
  if (!GAME_KEYS.includes(game)) throw new Error("Unsupported game");
  ...
  if (game === "wheel") return wheel(wager, fair, params);
  ...
  return war(wager, fair, params);
}

export function dice(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  const target = roundMoney(numberParam(params, "target", 50.5, 2, 98));
  ...
  return outcome(wager, won ? multiplier : 0, won ? "win" : "loss", `${roll.toFixed(2)} rolled ${direction} ${target}`, { game: "dice", roll, target, direction, chance, multiplier });
}

export function limboMultiplier(fair: FairInput) {
  const raw = Math.max(rollFloat(fair), 0.000001);
  return Math.min(1000000, Math.floor(((1 - HOUSE_EDGE.limbo) / raw) * 100) / 100);
}

export function limbo(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  const target = roundMoney(numberParam(params, "target", 2, 1.01, 10000));
  const resultMultiplier = limboMultiplier(fair);
  const won = resultMultiplier >= target;
  return outcome(wager, won ? target : 0, won ? "win" : "loss", `${resultMultiplier.toFixed(2)}x result`, { game: "limbo", target, resultMultiplier });
}
...
export function minesInstant(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  ...
  return outcome(wager, multi, busted ? "loss" : "cashout", busted ? `Mine hit after ${safe} safe picks` : `${safe} safe picks cashed out`, { game: "mines", mineCount, mines, selected, safe });
}
...
export function towerInstant(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  ...
  const width = 3;
  const traps = towerLayout(fair, rows, width);
  const picks = Array.from({ length: rows }, (_, row) => rollInt({ ...fair, cursor: row + 200 }, width));
  const bustRow = picks.findIndex((pick, row) => pick === traps[row]);
  const cleared = bustRow === -1 ? rows : bustRow;
  const multi = bustRow === -1 ? towerMultiplier(cleared, width) : 0;
  return outcome(wager, multi, bustRow === -1 ? "cashout" : "loss", bustRow === -1 ? `Cleared ${rows} rows` : `Trap on row ${bustRow + 1}`, { game: "tower", rows, width, traps, picks, cleared });
}
...
export function plinko(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  ...
  const multipliers = PLINKO_TABLES[risk];
  ...
  return outcome(wager, multi, multi >= 1 ? "win" : "loss", `Bucket ${bucket + 1} paid ${multi}x`, { game: "plinko", risk, path, bucket, multiplier: multi });
}

export const WHEEL_SEGMENTS = [0, 0, 0.5, 1.2, 1.5, 2, 3, 5, 10, 20];

export function wheel(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  const segment = rollInt(fair, WHEEL_SEGMENTS.length);
  const multi = WHEEL_SEGMENTS[segment];
  return outcome(wager, multi, multi > 1 ? "win" : "loss", `Segment ${segment + 1} paid ${multi}x`, { game: "wheel", segment, segments: WHEEL_SEGMENTS, multiplier: multi });
}

export const KENO_TABLES: Record<number, number[]> = {
  1: [0, 3.8],
  2: [0, 1.8, 5],
  3: [0, 0, 2.5, 25],
  4: [0, 0, 1.2, 5, 80],
...
export function normalizeKenoPicks(params?: Record<string, unknown>) {
  const raw = Array.isArray(params?.picks) ? params?.picks as unknown[] : [1, 7, 13, 21, 33];
  return Array.from(new Set(raw.map(Number).filter((n) => Number.isInteger(n) && n >= 1 && n <= 40))).slice(0, 10);
}

export function keno(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  const picks = normalizeKenoPicks(params);
  if (picks.length < 1) return outcome(wager, 0, "loss", "No valid picks", { game: "keno", picks });
  const draw = sampleWithoutReplacement(fair, 40, 10).map((n) => n + 1);
  const hits = picks.filter((pick) => draw.includes(pick)).length;
  const table = KENO_TABLES[picks.length];
  const multi = table[Math.min(hits, table.length - 1)] ?? 0;
  return outcome(wager, multi, multi > 1 ? "win" : "loss", `${hits} hits from ${picks.length} picks`, { game: "keno", picks, draw, hits, multiplier: multi });
}

export function hiloDeck(fair: FairInput, count = 20) {
  return sampleWithoutReplacement(fair, 52, count);
}
...
export function hilo(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  ...
  const deck = hiloDeck(fair, 4);
  const firstCard = deck[0];
  const secondCard = deck[1];
  const first = cardRank(firstCard);
  const second = cardRank(secondCard);
  const won = choice === "higher" ? second > first : second < first;
  const push = second === first;
  if (push) return outcome(wager, 1, "push", `Push: ${cardName(firstCard)} to ${cardName(secondCard)}`, { game: "hilo", deck, firstCard: cardName(firstCard), secondCard: cardName(secondCard), first, second, choice });
  return outcome(wager, won ? 1.94 : 0, won ? "win" : "loss", `${cardName(firstCard)} to ${cardName(secondCard)}, picked ${choice}`, { game: "hilo", deck, firstCard: cardName(firstCard), secondCard: cardName(secondCard), first, second, choice });
}

export function crashPoint(fair: FairInput) {
  const raw = Math.max(rollFloat(fair), 0.000001);
  return Math.min(10000, Math.floor(((1 - HOUSE_EDGE.crash) / raw) * 100) / 100);
}

export function crash(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  const autoCashout = roundMoney(numberParam(params, "autoCashout", 2, 1.01, 1000));
  const crashAt = crashPoint(fair);
  const won = crashAt >= autoCashout;
  return outcome(wager, won ? autoCashout : 0, won ? "cashout" : "loss", `Crash point ${crashAt.toFixed(2)}x`, { game: "crash", autoCashout, crashAt });
}

export function blackjackDeck(fair: FairInput) {
  return sampleWithoutReplacement(fair, 52, 20);
}
...
export function blackjack(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  ...
  if (player > 21) return outcome(wager, 0, "loss", `Bust ${player}`, { game: "blackjack", deck, playerCards: playerCards.map(cardName), dealerCards: dealerCards.map(cardName), player, action });
  ...
  if (dealer > 21) return outcome(wager, natural ? 2.5 : action === "double" ? 4 : 2, "win", `Dealer bust ${dealer}`, { game: "blackjack", deck, playerCards: playerCards.map(cardName), dealerCards: dealerCards.map(cardName), player, dealer, action, natural });
  if (player === dealer) return outcome(wager, 1, "push", `Push ${player}`, { game: "blackjack", deck, playerCards: playerCards.map(cardName), dealerCards: dealerCards.map(cardName), player, dealer, action, natural });
  const won = player > dealer;
  const winMulti = natural ? 2.5 : action === "double" ? 4 : 2;
  return outcome(wager, won ? winMulti : 0, won ? "win" : "loss", `Player ${player}, dealer ${dealer}`, { game: "blackjack", deck, playerCards: playerCards.map(cardName), dealerCards: dealerCards.map(cardName), player, dealer, action, natural });
}
...
export function rps(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  ...
  if (pick === house) return outcome(wager, 1, "push", `Both picked ${pick}`, { game: "rps", pick, house });
  return outcome(wager, win ? 1.98 : 0, win ? "win" : "loss", `${pick} against ${house}`, { game: "rps", pick, house });
}

export function war(wager: number, fair: FairInput, params?: Record<string, unknown>) {
  const deck = sampleWithoutReplacement(fair, 52, 2);
  const playerCard = deck[0];
  const dealerCard = deck[1];
  const player = cardRank(playerCard);
  const dealer = cardRank(dealerCard);
  if (player === dealer) return outcome(wager, 1, "push", `Tie at ${cardName(playerCard)}`, { game: "war", deck, playerCard: cardName(playerCard), dealerCard: cardName(dealerCard), player, dealer });
  const won = player > dealer;
  return outcome(wager, won ? 1.98 : 0, won ? "win" : "loss", `Player ${cardName(playerCard)}, dealer ${cardName(dealerCard)}`, { game: "war", deck, playerCard: cardName(playerCard), dealerCard: cardName(dealerCard), player, dealer });
}

export function compareOutcome(a: GameOutcome, b: GameOutcome) {
